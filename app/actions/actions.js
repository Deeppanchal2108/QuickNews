...
export async function fetchCategoryNews(category, page=1) {
    try {
        ...
        const response = await fetch(`https://newsapi.org/v2/everything?q=${category}&from=2025-04-15&sortBy=popularity&language=en&pageSize=7&page=${page}&apiKey=${process.env.NEXT_PUBLIC_NEWS_API_KEY}`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${process.env.NEXT_PUBLIC_NEWS_API_KEY}`,
            },
